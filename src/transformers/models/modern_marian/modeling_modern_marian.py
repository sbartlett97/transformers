...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    # Handle Grouped Query Attention: repeat key and value if needed
...
class MarianGroupedQueryAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if is_cross_attention and past_key_values is not None and is_updated:
            ...
            # Key/Value projections: reduced number of heads for GQA
            kv_input_shape = (bsz, src_len, self.num_key_value_heads, self.head_dim)
            key_states = self.k_proj(current_states)
            value_states = self.v_proj(current_states)
            key_states = key_states.view(*kv_input_shape).transpose(1, 2)
            value_states = value_states.view(*kv_input_shape).transpose(1, 2)
